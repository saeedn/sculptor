...
## Database simplification (2026-06-26, fifth batch) — clean break

Two clean-break DB changes (each `just check` + `just test-unit` green):

1. **Squashed all migrations into one initial migration.** Deleted the 28
   incremental migrations + their 29 data-preservation fixtures; replaced with
   a single autogenerated `initial schema` migration (down_revision=None) + one
   fixture. The alembic framework, frozen-JSON-schema flow, and
   `bump_migrations.py` are kept, so incremental migrations still work going
   forward — this just resets the baseline. NOTE: `bump_migrations.py` can't
   generate the squash (it queries a not-yet-created DB); drive
   `command.revision(autogenerate=True)` directly against an EMPTY connection
   instead (see the scratchpad `gen_initial.py`). Do NOT use
   `METADATA.create_all()` inside the migration — it isn't version-pinned and
   would break the next incremental migration.

2. **Collapsed the two-table append-only pattern to single mutable tables.**
   Each model was a snapshots table (insert-only source of truth) + a
   trigger-maintained `<name>_latest` table. Nothing read the snapshot history
   (reads hit `_latest`; change-detection is in-memory), so collapsed to one
   mutable table keyed by `object_id`. The former `before_insert` trigger's
   upsert — including `MAX()` latching for monotonic columns (the SCU-168
   soft-delete revive guard) — moved into `_upsert_model` as a real
   `INSERT ... ON CONFLICT(object_id) DO UPDATE`. Removed the trigger machinery
   (`drop_all_automanaged_triggers`, the init-time create-triggers loop) and
   the audit-snapshot write in `_update_model_fields`. Regenerated the initial
   migration (now 6 single tables, no `_latest`/`snapshot_id`).
