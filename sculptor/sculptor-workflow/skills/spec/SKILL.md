...
also any step that asks the user a question. The non-negotiable rule:
**every turn in a Q&A loop MUST end by asking the user a question with
your question tool** — ending a turn without it is a silent stop and
...
and how to ask (1-4 grounded options; the free-text field always
exists).

Spec-specific deltas on top of the shared ritual:

- The artifact you update after every answer is **the spec file**.
- The spawn-turn exception applies when you spawn a **Mock agent**
  (Step 6, Step 7's escape hatch, or Step 9) or an **Architect agent**
  (Step 9): that turn ends with text instructions, not a question.
