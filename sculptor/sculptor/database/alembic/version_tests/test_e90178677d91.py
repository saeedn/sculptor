...
class TestMigratione90178677d91(MigrationTestFixture):
    """Test fixture for migration e90178677d91 (drop workspace.setup_command_triggered).
        ...
        connection.execute(
            sa.text(
                ...
                INSERT INTO workspace (
                    ...
                    setup_command_triggered, setup_status, setup_log_truncated, diff_status
                ) VALUES (
                    '2026-01-01 00:00:00', 'workspace-1', 'project-1', 'org-1',
                    'a workspace', 'fork', 0, 1, 1, 'pending', 0, 'clean'
                )
                """
            )
        )
    ...
    def verify(self, connection: sa.engine.Connection) -> None:
        ...
        assert "setup_command_triggered" not in columns, "setup_command_triggered column should be dropped"
